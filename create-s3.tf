...
resource "aws_s3_bucket_lifecycle_configuration" "com_div_proj_comp_s3_bucket_lifecycle" {
  bucket = aws_s3_bucket.com_div_proj_comp_s3_bucket.id
...
resource "aws_s3_bucket" "com_div_proj_comp_s3_LOG_BUCKET" {
  bucket_prefix = "com.div.proj.comp.s3-log-bucket"  # Replace with a unique bucket name
}
resource "aws_s3_bucket_server_side_encryption_configuration" "com_div_proj_comp_s3_LOG_BUCKET-sse" {
  bucket = aws_s3_bucket.com_div_proj_comp_s3_LOG_BUCKET.id
  rule {
    apply_server_side_encryption_by_default {
      kms_master_key_id = aws_kms_key.mykey.arn
      sse_algorithm     = "aws:kms"
...
resource "aws_s3_bucket_versioning" "com_div_proj_comp_s3_LOG_BUCKET_versioning" {
  bucket = aws_s3_bucket.com_div_proj_comp_s3_LOG_BUCKET.id
  versioning_configuration {
    status = "Enabled"
  }
}
# resource "aws_s3_bucket_acl" "com_div_proj_comp_s3_LOG_BUCKET-acl" {
#   bucket = aws_s3_bucket.com_div_proj_comp_s3_bucket.id
#   acl    = "log-delivery-write"
# }
resource "aws_s3_bucket_public_access_block" "com_div_proj_comp_s3_LOG_BUCKET_block_public_access" {
  bucket = aws_s3_bucket.com_div_proj_comp_s3_LOG_BUCKET.id
  block_public_acls   = true
  block_public_policy = true
  restrict_public_buckets = true
...
resource "aws_s3_bucket_lifecycle_configuration" "com_div_proj_comp_s3_LOG_BUCKET_lifecycle" {
  bucket = aws_s3_bucket.com_div_proj_comp_s3_LOG_BUCKET.id
